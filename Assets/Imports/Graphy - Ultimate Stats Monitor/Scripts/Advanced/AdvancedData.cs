    ...
    public class AdvancedData : MonoBehaviour, IMovable, IModifiableState
    {
        ...
        void Update()
        {
            ...
            if (m_deltaTime > 1.0 / m_updateRate)
            {
                ...
                m_sb.Append("Window: ").Append(Screen.width).Append("x").Append(Screen.height)
                    .Append("@").Append(Screen.currentResolution.refreshRate).Append("Hz")
                    .Append("[").Append(Screen.dpi).Append("dpi]");
